---
name: empathy-map-template-generator
render_family: board
description: Generate a reusable empathy-map template. Use when you need a blank or lightly example-filled empathy-map board for workshops before project evidence is added.
---

# Empathy Map Template Generator

Generate a reusable empathy-map board template with prompts for each empathy section.


Use this skill when the team needs a workshop-ready template or blank canvas. Use `empathy-map-facilitator` when the artifact should already include facilitation framing for a live session.

This skill is opt-in only. Do not use it as a fallback when a user asked for a real empathy map but has not provided the required actor or evidence yet.
...
- placeholder actor or actor label

Optional inputs:

- whether to include example cards
- a preferred artifact slug
## Generate The Template

Create a board with prompt cards for:
...
- makes evidence prompts explicit,
- clearly distinguishes prompt/example cards from real evidence,
- and aligns with the empathy workshop and builder skills.
...
- Only generate this template when the user explicitly asks for a template, blank canvas, workshop starter, or chooses that option after coaching.
